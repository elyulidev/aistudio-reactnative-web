...
export const conference41: CurriculumTopic = {
    ...
    content: [
        ...
        { type: 'paragraph', text: 'En este proyecto de dos partes, construiremos una aplicación de listado de propiedades inmobiliarias. Toda la información se gestionará localmente usando SQLite y Drizzle ORM. En esta primera parte, nos centraremos en la configuración, el modelo de datos, y la creación de la pantalla principal que muestra y filtra las propiedades.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 1: Configuración del Proyecto' },
        { type: 'paragraph', text: 'Crea un nuevo proyecto Expo e instala todas las dependencias que hemos aprendido a usar.' },
...
`},
        { type: 'paragraph', text: 'Sigue los pasos de las conferencias anteriores para configurar Drizzle (`drizzle.config.ts`, `package.json scripts`), NativeWind (`tailwind.config.js`, `babel.config.js`, `global.css`), y la importación de CSS en `app/_layout.tsx`.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 2: Definir el Esquema de la Base de Datos' },
        { type: 'paragraph', text: 'Crea el archivo `db/schema.ts` para definir la tabla de propiedades.' },
...
`},
        ...
        { type: 'paragraph', text: 'Crea un archivo `db/seed.ts` para poblar la base de datos con algunas propiedades de ejemplo. Esto es crucial para poder desarrollar la UI sin tener que añadir datos manualmente.' },
        { type: 'code', language: 'typescript', code: `
// db/seed.ts
import { db } from './';
...
`},
        { type: 'callout', alertType: 'info', text: 'Para las imágenes, usaremos URLs de Unsplash por simplicidad. En la segunda parte, las reemplazaremos con imágenes del sistema de archivos local.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 4: Construir la Pantalla de Listado (`app/index.tsx`)' },
        { type: 'paragraph', text: 'Esta pantalla mostrará los filtros y la lista de propiedades.' },
...
`},
        { type: 'paragraph', text: 'En la siguiente conferencia, construiremos el componente `PropertyCard`, la pantalla de detalles y el formulario para añadir nuevas propiedades.' }
    ]
};
