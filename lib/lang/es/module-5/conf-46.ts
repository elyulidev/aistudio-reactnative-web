...
export const conference46: CurriculumTopic = {
    ...
    content: [
        ...
        { type: 'paragraph', text: 'En este proyecto, construiremos un prototipo de una aplicación de red social. Crearemos un feed donde se podrán mostrar publicaciones con texto e imagen, y los usuarios podrán dar "like". Toda la información, incluyendo los posts y los likes, se almacenará y gestionará localmente en la base de datos SQLite con Drizzle.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 1: Definir el Esquema de la Base de Datos' },
        { type: 'paragraph', text: 'Necesitamos dos tablas: una para los posts y otra para registrar los likes de cada post. La relación será "un post puede tener muchos likes".' },
...
`},
        { type: 'paragraph', text: 'No olvides generar la nueva migración: `npm run generate`.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 2: Crear el Formulario de Nuevo Post' },
        { type: 'paragraph', text: 'Crea una nueva pantalla modal para que los usuarios puedan crear posts. Usa `expo-image-picker` y `expo-file-system` para manejar la selección y guardado de imágenes.' },
...
`},
        ...
        { type: 'paragraph', text: 'La pantalla principal (`app/index.tsx`) buscará todos los posts y sus likes, y los mostrará en una `FlatList`.' },
        
        { type: 'paragraph', text: '**La Consulta con `with` para Relaciones:**' },
        { type: 'paragraph', text: 'Para obtener los posts y sus likes relacionados de manera eficiente, usamos la sintaxis `with` de Drizzle, que realiza un JOIN por debajo.' },
        { type: 'code', language: 'typescript', code: `
...
`},
        ...
        { type: 'paragraph', text: 'Crea un componente reutilizable para mostrar cada post. Este componente también manejará la lógica para dar "like".' },
        { type: 'code', language: 'tsx', code: `
...
`},
        { type: 'callout', alertType: 'tip', text: 'Pasar una función `refetch` como prop al `PostCard` es una forma simple de hacer que el feed se actualice inmediatamente después de dar like. En aplicaciones más complejas, se usarían soluciones de gestión de estado más sofisticadas.'}
    ]
};
