...
export const conference40: CurriculumTopic = {
    ...
    content: [
        ...
        { type: 'paragraph', text: 'Escribir código que funciona es solo el primer paso. Escribir código limpio —código que es legible, mantenible y fácil de modificar— es una habilidad esencial para el desarrollo profesional. La refactorización es el proceso de reestructurar el código existente, sin cambiar su comportamiento externo, para mejorar estos atributos.' },
        { type: 'divider' },
        ...
        { type: 'paragraph', text: 'Los "Code Smells" (malos olores en el código) son síntomas de que algo podría estar mal en tu código y podría beneficiarse de una refactorización.' },
        { type: 'list', items: [
        ...
        { type: 'subtitle', text: '3. Ejercicio Práctico: Refactorizando una Pantalla de Búsqueda' },
        { type: 'paragraph', text: 'Tomemos la pantalla de búsqueda de películas del módulo anterior. Aunque funcional, mezcla la lógica de la API, el manejo del estado y la renderización de la UI en un solo componente. Vamos a refactorizarla.' },
        
        { type: 'paragraph', text: '**ANTES: El Componente Monstruoso**' },
        { type: 'paragraph', text: 'Todo está en `app/index.tsx`. Es difícil de leer y aún más difícil de reutilizar cualquier parte de su lógica.' },
...
`},
        ...
        { type: 'paragraph', text: 'Separaremos las responsabilidades en archivos más pequeños y manejables.' },

        { type: 'paragraph', text: '**1. Mover la Lógica a un Hook Personalizado (`hooks/useMovieSearch.ts`)**' },
...
`},
        { type: 'paragraph', text: '**2. Crear un Componente de UI Reutilizable (`components/MovieListItem.tsx`)**' },
        { type: 'paragraph', text: 'Esto ya lo hicimos en el proyecto del módulo 4, demostrando que ya estábamos aplicando buenos principios.'},

        { type: 'paragraph', text: '**3. El Componente de la Pantalla Ahora es Limpio y Declarativo**' },
        { type: 'code', language: 'tsx', code: `
...
`},
        { type: 'callout', alertType: 'tip', text: 'Observa cómo la pantalla ahora solo se preocupa de la UI y de conectar el estado del hook con los componentes. La lógica de búsqueda es completamente independiente y podría ser reutilizada en otra parte de la app.'}
    ]
};
