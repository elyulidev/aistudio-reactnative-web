...
export const conference8: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: '¡Es hora de poner en práctica todo lo que hemos aprendido en el Módulo 1! En este mini proyecto, construiremos un componente de UI reutilizable y visualmente atractivo: una tarjeta de película. Este ejercicio consolidará tus conocimientos sobre componentes, layout con Flexbox, estilización con NativeWind y gestión de assets.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/movie-card-project-goal.png', caption: 'El componente `MovieCard` que construiremos.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Paso 1: Preparación del Componente y Assets' },
      { type: 'paragraph', text: 'Primero, creemos el archivo para nuestro nuevo componente. Dentro de tu proyecto, crea una carpeta `components` (si aún no existe) y dentro de ella, un nuevo archivo llamado `MovieCard.tsx`.' },
      { type: 'paragraph', text: 'Copia el siguiente código base en tu nuevo archivo. Este será nuestro punto de partida.' },
...
` },
      { type: 'callout', alertType: 'info', text: 'Para la imagen del póster, usaremos una URL. En un proyecto real, podrías obtener estos datos de una API o de tus assets locales.' },
      
      { type: 'divider' },

      { type: 'subtitle', text: 'Paso 2: Estructura Principal y Estilo del Contenedor' },
      { type: 'paragraph', text: 'Vamos a empezar por el contenedor principal. Queremos que tenga un fondo oscuro, esquinas redondeadas y una sombra para que parezca una tarjeta. Usaremos NativeWind para esto.' },
      { type: 'code', language: 'jsx', code: `
...
`},
      ...
      { type: 'paragraph', text: 'Ahora, añadamos la imagen del póster dentro de nuestro contenedor. Haremos que ocupe todo el ancho de la tarjeta y le daremos una altura fija.' },
      { type: 'code', language: 'jsx', code: `
// ...
...
`},
      ...
      { type: 'paragraph', text: 'Debajo de la imagen, crearemos una sección con el título, el género y la calificación. Usaremos un `<View>` con padding y flexbox para organizar el contenido.' },
      { type: 'code', language: 'jsx', code: `
// ...
...
` },
      ...
      { type: 'paragraph', text: 'Para completar nuestra tarjeta, añadiremos un botón "Ver Detalles". Usaremos el componente `<Pressable>` para que sea interactivo y lo estilizaremos para que se destaque.' },
      { type: 'code', language: 'jsx', code: `
...
`},
      ...
      { type: 'paragraph', text: '¡Felicidades! Has construido un componente complejo y reutilizable. Aquí está el código completo para `MovieCard.tsx`:' },
      { type: 'code', language: 'jsx', code: `
import React from 'react';
import { View, Text, Image, Pressable } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
...
export default MovieCard;
`},
      { type: 'paragraph', text: 'Para verlo en acción, importa y usa tu nuevo componente en tu pantalla principal (por ejemplo, `app/index.tsx`):' },
      { type: 'code', language: 'jsx', code: `
...
`},
      { type: 'paragraph', text: 'Este mini proyecto demuestra cómo combinar diferentes componentes y utilidades de estilo para crear interfaces de usuario pulidas y profesionales. ¡Excelente trabajo!' }
    ]
};
