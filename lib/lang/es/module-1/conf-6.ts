...
export const conference6: CurriculumTopic = {
    ...
    content: [
      { type: 'heading', text: 'Estilización Avanzada con NativeWind y Tailwind CSS' },
      { type: 'paragraph', text: '¡Bienvenidos a esta sesión avanzada sobre estilización en React Native! Hoy vamos a explorar NativeWind, una herramienta que nos permite integrar el popular framework Tailwind CSS directamente en nuestras aplicaciones React Native, ofreciéndonos un flujo de trabajo de "utilidad primero" para una estilización rápida, consistente y altamente mantenible.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Instalación y Configuración de NativeWind' },
      { type: 'paragraph', text: 'La configuración oficial para un proyecto de Expo es la siguiente:' },
      { type: 'list', items: [
      ...
      { type: 'code', language: 'bash', code: `npm install nativewind react-native-reanimated@~3.17.4 react-native-safe-area-context@5.4.0\nnpm install --dev tailwindcss@^3.4.17 prettier-plugin-tailwindcss@^0.5.11` },
      { type: 'list', items: [
      ...
      { type: 'paragraph', text: 'A continuación, crea un archivo llamado `global.css` en la raíz de tu proyecto y añade las directivas de Tailwind.' },
      { type: 'code', language: 'css', code: `/* global.css */\n@tailwind base;\n@tailwind components;\n@tailwind utilities;` },
      { type: 'list', items: [
      ...
      { type: 'code', language: 'javascript', code: `// babel.config.js\nmodule.exports = function (api) {\n  api.cache(true);\n  return {\n    presets: [\n      ["babel-preset-expo", { jsxImportSource: "nativewind" }],\n      "nativewind/babel",\n    ],\n  };\n};` },
      { type: 'list', items: [
      ...
      { type: 'code', language: 'typescript', code: `/// <reference types="nativewind/types" />` },
      { type: 'callout', alertType: 'warning', text: '¡Importante! Después de la configuración, reinicia completamente el servidor de desarrollo (\`npx expo start --clear\`) para asegurarte de que todos los cambios se apliquen correctamente.' },
      
      { type: 'divider' },

      { type: 'subtitle', text: '2. Uso de Clases de Tailwind CSS' },
      { type: 'paragraph', text: 'Con NativeWind configurado, podemos aplicar estilos a nuestros componentes de React Native utilizando la prop `className`, de forma muy similar a como lo haríamos en la web. Esto permite un desarrollo rápido y una interfaz consistente.' },
      { type: 'code', language: 'jsx', code: `import { Text, View } from 'react-native';\n\nexport default function App() {\n  return (\n    <View className="flex-1 items-center justify-center bg-white">\n      <Text className="text-xl font-bold text-blue-500">\n        ¡Hola, NativeWind!\n      </Text>\n    </View>\n  );\n}` },

      { type: 'divider' },

      { type: 'subtitle', text: '3. IntelliSense y Personalización del Tema' },
      { type: 'paragraph', text: 'La mayoría de los IDEs (como VSCode con la extensión de Tailwind CSS) ofrecen autocompletado y sugerencias inteligentes para las clases de Tailwind. Además, puedes extender el tema en `tailwind.config.js` para definir tu propia paleta de colores, tipografías y más.' },
      { type: 'code', language: 'javascript', code: `// tailwind.config.js\nmodule.exports = {\n  // ...\n  theme: {\n    extend: {\n      colors: {\n        primary: '#FF6363',\n        'dark-200': '#2A2A2A',\n      },\n      fontFamily: {\n        'quicksand-bold': ['Quicksand-Bold', 'sans-serif'],\n      }\n    },\n  },\n  // ...\n};`},
      { type: 'paragraph', text: 'Una vez definidos, puedes usarlos como `text-primary`, `bg-dark-200` o `font-quicksand-bold`.' },

      { type: 'divider' },
      
      { type: 'subtitle', text: '4. Gestión de `globals.css`' },
      { type: 'paragraph', text: 'Aunque Tailwind es "utilidad primero", a veces querrás crear tus propias clases compuestas o "componentes" que encapsulen varias utilidades. Puedes definirlas en `global.css` para una fácil reutilización.' },
      { type: 'code', language: 'css', code: `@tailwind base;\n@tailwind components;\n@tailwind utilities;\n\n@layer components {\n  .h1-bold {\n    @apply text-3xl font-bold text-gray-900;\n  }\n}` },
      { type: 'paragraph', text: 'Luego, puedes usar `className="h1-bold"` en tu JSX, manteniendo el código más limpio.' },

      { type: 'divider' },
      
      { type: 'subtitle', text: '5. Tarea Práctica: Refactorización a NativeWind' },
      { 
        type: 'assignment',
