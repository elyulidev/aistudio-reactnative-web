...
export const conference16: CurriculumTopic = {
    ...
    content: [
      { type: 'heading', text: 'Mini Proyecto: App de Blog con Navegación Completa' },
      { type: 'paragraph', text: '¡Es hora de consolidar todo lo aprendido en el Módulo 2! En este proyecto, construiremos una aplicación de blog simple pero completa. Integraremos un Stack Navigator, un Tab Navigator, rutas dinámicas para los artículos y una estructura de archivos lógica usando grupos de rutas.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/blog-app-nav-structure.png', caption: 'Estructura de navegación de nuestra aplicación de blog.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Paso 1: Estructura de Archivos del Proyecto' },
      { type: 'paragraph', text: 'Una buena estructura de archivos es clave. Vamos a organizar nuestras rutas de la siguiente manera dentro de la carpeta `app`:' },
...
└── _layout.tsx          # Define el Stack Navigator raíz
      ...
      { type: 'paragraph', text: 'Modifiquemos `app/_layout.tsx` para definir nuestro Stack principal. Gestionará el grupo de pestañas y la pantalla de detalles de los posts.' },
...
export default function RootLayout() {
  return (
    ...
    { type: 'callout', alertType: 'info', text: 'Ocultamos el encabezado para `(tabs)` para que el Tab Navigator pueda gestionar sus propios encabezados, evitando una doble barra de título.' },
    { type: 'divider' },
    
    { type: 'subtitle', text: 'Paso 3: Configurar el Tab Navigator' },
    { type: 'paragraph', text: 'Ahora, configuremos las pestañas en `app/(tabs)/_layout.tsx`.' },
    { type: 'code', language: 'jsx', code: `
...
export default function TabLayout() {
  return (
    ...
    { type: 'paragraph', text: 'La pantalla `index` en nuestras pestañas mostrará la lista de artículos. Usaremos `FlatList` y `<Link>` para navegar a los detalles. Para mantenerlo simple, los datos estarán directamente en el archivo.' },
    { type: 'code', language: 'jsx', code: `
// app/(tabs)/index.tsx
...
export default function ArticlesScreen() {
  return (
    ...
    { type: 'callout', alertType: 'tip', text: 'El prop `asChild` en el componente `<Link>` le permite pasar sus propiedades de navegación al primer componente hijo (`Pressable`), haciendo que toda el área sea clicable.'},
    { type: 'divider' },

    { type: 'subtitle', text: 'Paso 5: Crear la Pantalla "Acerca de"' },
    { type: 'paragraph', text: 'Esta será una pantalla estática simple en `app/(tabs)/about.tsx`.' },
    { type: 'code', language: 'jsx', code: `
// app/(tabs)/about.tsx
...
export default function AboutScreen() {
  return (
    ...
    { type: 'subtitle', text: 'Paso 6: Crear la Pantalla de Detalles Dinámica' },
    { type: 'paragraph', text: 'Finalmente, la pantalla `app/posts/[id].tsx`. Usaremos `useLocalSearchParams` para obtener el ID de la URL y mostrar el artículo correcto.' },
    { type: 'code', language: 'jsx', code: `
// app/posts/[id].tsx
import { View, Text } from 'react-native';
...
export default function PostDetailsScreen() {
  ...
  return (
    ...
    { type: 'paragraph', text: 'Has construido con éxito una aplicación con una estructura de navegación completa. Has combinado un Stack Navigator con un Tab Navigator, has usado grupos de rutas para organizar tu código, has creado rutas dinámicas y has pasado datos entre pantallas. ¡Este es el fundamento de casi cualquier aplicación móvil compleja!' }
    ]
};
