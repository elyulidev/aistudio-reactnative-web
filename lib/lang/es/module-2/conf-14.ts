...
export const conference14: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Un caso de uso muy común es tener un estado booleano que se alterna (como para un modal o un menú desplegable). En lugar de escribir `const [isOpen, setIsOpen] = useState(false);` y la función `toggle` cada vez, podemos crear un hook para ello.' },
      { type: 'paragraph', text: 'Crea una carpeta `hooks` en la raíz de tu proyecto y dentro, un archivo `useToggle.ts`.' },
...
`},
    { type: 'paragraph', text: 'Ahora, en cualquier componente, podemos usarlo de forma mucho más limpia:' },
...
`},
      ...
      { type: 'paragraph', text: 'En una barra de búsqueda, no queremos hacer una petición a la API con cada letra que el usuario escribe. Queremos esperar a que el usuario termine de escribir. El "debouncing" es la técnica perfecta para esto, y podemos encapsularla en un hook.' },
      { type: 'code', language: 'typescript', code: `
...
`},
    { type: 'paragraph', text: 'Así lo usaríamos en un componente de búsqueda:' },
    { type: 'code', language: 'jsx', code: `
...
`},
    ...
    { type: 'subtitle', id: 'async-storage', text: '4. Un Vistazo al Almacenamiento Local: AsyncStorage' },
    { type: 'paragraph', text: 'Antes de nuestra tarea práctica, necesitamos una forma de guardar datos en el dispositivo del usuario. Para datos simples, como las preferencias del usuario o un token de sesión, `AsyncStorage` es la herramienta perfecta. Es un sistema de almacenamiento de clave-valor, asíncrono y persistente.' },
    { type: 'list', items: [
...
`},
    { type: 'callout', alertType: 'tip', text: 'Con este conocimiento sobre `AsyncStorage`, ahora tienes todo lo necesario para abordar la siguiente tarea práctica.' },
    
    { type: 'subtitle', text: '5. Tarea Práctica: Crear un Hook `usePersistentState`' },
    { type: 'paragraph', text: 'Ahora vamos a crear un hook que actúe como `useState` pero que además guarde el valor en el almacenamiento local del dispositivo usando `AsyncStorage`. Esto es increíblemente útil para recordar preferencias del usuario, como el tema de la aplicación.' },
    { 
        type: 'assignment',
