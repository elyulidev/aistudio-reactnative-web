...
export const conference33: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'En esta segunda parte del proyecto, construiremos la pantalla de detalles para una película seleccionada y añadiremos una funcionalidad clave: guardar películas como "favoritas" en nuestra base de datos local SQLite usando Drizzle ORM.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Paso 1: Esquema y Migración para Favoritos' },
      { type: 'paragraph', text: 'Primero, necesitamos una tabla en nuestra base de datos para almacenar las películas favoritas. Abramos `db/schema.ts` y añadamos una nueva tabla.' },
...
`},
      { type: 'paragraph', text: 'Hemos usado el ID de la película de la API como clave primaria para evitar duplicados. Ahora, genera la migración:' },
      { type: 'code', language: 'bash', code: 'npm run generate' },
      { type: 'callout', alertType: 'info', text: 'Recuerda que las migraciones se aplicarán automáticamente al reiniciar la app gracias al hook `useMigrations` en tu layout raíz.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: 'Paso 2: Crear la Ruta Dinámica para Detalles' },
      { type: 'paragraph', text: 'Necesitamos una nueva pantalla que pueda mostrar los detalles de cualquier película. Creamos una ruta dinámica en `app/movie/[id].tsx`.' },
...
...
`},
      { type: 'paragraph', text: 'Registra esta nueva ruta en tu Stack Navigator raíz (`app/_layout.tsx`).' },
...
`},
      ...
      { type: 'paragraph', text: 'Modifiquemos nuestro `MovieListItem` para que, al ser presionado, navegue a la pantalla de detalles, pasando el ID de la película.' },
      { type: 'code', language: 'tsx', code: `
...
`},
      ...
      { type: 'subtitle', text: 'Paso 4: Construir la Pantalla de Detalles y Lógica de Favoritos' },
      { type: 'paragraph', text: 'Esta es la parte central. En `app/movie/[id].tsx`, obtendremos el ID, cargaremos los datos de la película (podríamos hacer otra llamada a la API para más detalles, pero por simplicidad, pasaremos los datos básicos), y manejaremos la lógica de favoritos.' },
      { type: 'code', language: 'tsx', code: `
// app/movie/[id].tsx
