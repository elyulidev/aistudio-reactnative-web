...
export const conference31: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Más allá del renderizado básico, `FlatList` ofrece un potente conjunto de características para crear experiencias de usuario modernas y fluidas, como la carga infinita de datos, la capacidad de refrescar la lista y la adición de cabeceras y pies de página.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Carga Infinita (Infinite Scrolling) con `onEndReached`' },
      { type: 'paragraph', text: 'En lugar de cargar miles de elementos a la vez, una mejor estrategia es cargar un lote inicial (ej. 20 elementos) y cargar más a medida que el usuario se desplaza hacia el final de la lista. Esto se logra con la prop `onEndReached`.' },
      { type: 'list', items: [
...
`},
      ...
      { type: 'paragraph', text: 'Una experiencia de usuario común en aplicaciones móviles es la capacidad de "tirar" de la parte superior de una lista para recargar los datos. `FlatList` soporta esto a través de la prop `refreshing` y `onRefresh`.' },
      { type: 'code', language: 'tsx', code: `
import React, { useState, useCallback } from 'react';
...
`},
      ...
      { type: 'subtitle', text: '3. Cabeceras y Pies de Página (`ListHeaderComponent`, `ListFooterComponent`)' },
      { type: 'paragraph', text: '`FlatList` permite añadir componentes React en la parte superior e inferior de la lista. Estos componentes se desplazan junto con la lista.' },
      { type: 'list', items: [
...
`},
      ...
      { type: 'heading', text: '¡Pon a prueba tus conocimientos!' },
        { 
          ...
          questions: [
            {
              "question": "¿Qué prop de `FlatList` se utiliza para implementar la carga infinita (infinite scroll)?",
              "options": ["`onScrollEnd`", "`onLoadMore`", "`onEndReached`", "`onScroll`"],
              "correctAnswer": 2
            },
            {
              "question": "Para implementar la funcionalidad de 'pull-to-refresh', ¿qué props de `FlatList` son esenciales?",
              "options": ["`loading` y `onLoad`", "`refreshing` y `onRefresh`", "`pulling` y `onPull`", "`reloading` y `onReload`"],
              "correctAnswer": 1
            },
            {
              "question": "Si quieres añadir una barra de búsqueda que se desplace junto con el contenido de la lista, ¿qué prop de `FlatList` deberías usar?",
              "options": ["`Header`", "`TopComponent`", "`ListHeaderComponent`", "Simplemente ponerlo antes de `FlatList` en el JSX"],
              "correctAnswer": 2
            },
            {
              "question": "La prop `onEndReachedThreshold` toma un valor numérico. ¿Qué representa un valor de 0.5?",
              "options": ["La función `onEndReached` se llamará cuando el usuario haya recorrido el 50% de la lista.", "La función `onEndReached` se llamará cuando el final del contenido esté a una distancia de media altura de la parte visible de la lista.", "La función `onEndReached` esperará 0.5 segundos antes de activarse.", "La función `onEndReached` se llamará cuando queden 50 elementos por mostrar."],
              "correctAnswer": 1
            }
          ]
        }
