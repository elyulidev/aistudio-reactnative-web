...
export const conference32: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'En este proyecto práctico, construiremos la primera parte de una aplicación para buscar películas. Integraremos una API externa (TheMovieDB), implementaremos una pantalla de búsqueda con `TextInput` y `debouncing`, y mostraremos los resultados de manera eficiente usando `FlatList`. Este proyecto consolidará todo lo aprendido sobre consumo de APIs y manejo de listas.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/movie-app-search.png', caption: 'La pantalla de búsqueda de películas que construiremos.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Parte 1: Obtener una Clave de API y Configurar el Entorno' },
      { type: 'paragraph', text: 'Para comunicarnos con The Movie Database (TMDB), primero necesitas una clave de API. Es gratis y esencial para nuestro proyecto.' },
      { type: 'list', items: [
...
`},
      { type: 'callout', alertType: 'warning', text: 'Asegúrate de añadir el archivo `.env` a tu `.gitignore` para no subir tus claves a repositorios públicos. Después de crear o modificar el archivo `.env`, debes reiniciar el servidor de desarrollo de Expo con el comando `npx expo start --clear` para que los cambios surtan efecto.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: 'Parte 2: Crear el Servicio de la API' },
      { type: 'paragraph', text: 'Es una buena práctica encapsular toda la lógica de las llamadas a la API en un solo lugar. Crea una nueva carpeta `services` y, dentro de ella, un archivo `api.ts`.' },
...
`},
    ...
    { type: 'paragraph', text: 'Para evitar hacer una llamada a la API cada vez que el usuario presiona una tecla, usaremos la técnica de "debouncing". Crearemos un hook personalizado para esto. Crea una carpeta `hooks` y dentro un archivo `useDebounce.ts`.'},
    { type: 'code', language: 'typescript', code: `
...
`},
      ...
      { type: 'paragraph', text: 'Para mantener nuestro código limpio, crearemos un componente separado para cada elemento de la lista. Crea una carpeta `components` y dentro un archivo `MovieListItem.tsx`.' },
...
`},
      ...
      { type: 'subtitle', text: 'Parte 5: Construir la Pantalla de Búsqueda Principal' },
      { type: 'paragraph', text: 'Finalmente, ensamblamos todo en nuestra pantalla principal. Modifica tu `app/index.tsx` para que contenga toda la lógica de estado y la UI de la búsqueda.' },
      { type: 'code', language: 'tsx', code: `
// app/index.tsx
...
`},
    { type: 'paragraph', text: '¡Y listo! Ahora tienes una pantalla de búsqueda funcional. Al escribir, la aplicación esperará 500ms, llamará a la API de TMDB y mostrará los resultados en una lista virtualizada, manejando los estados de carga y error. En la próxima conferencia, añadiremos la navegación a una pantalla de detalles.' }
    ]
};
