...
export const conference26: CurriculumTopic = {
    ...
    content: [
        { type: 'heading', text: 'Mini Proyecto: Aplicación To-Do Completa (Local)' },
        { type: 'paragraph', text: '¡Es hora de aplicar todo lo aprendido en el Módulo 3! En este proyecto, construiremos una aplicación de lista de tareas (To-Do) funcional desde cero. Implementaremos todas las operaciones CRUD (Crear, Leer, Actualizar, Borrar) utilizando Drizzle ORM para gestionar los datos de forma local en el dispositivo.' },
        { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/todo-app-project-goal.png', caption: 'El resultado final de nuestra aplicación de lista de tareas.' },
        { type: 'divider' },
        
        { type: 'subtitle', text: 'Paso 1: Creación y Configuración del Proyecto' },
        { type: 'paragraph', text: 'Empezaremos desde cero para simular un escenario real. Abre tu terminal y ejecuta los siguientes comandos:' },
...
`},
        { type: 'paragraph', text: 'A continuación, configura tu `package.json` con los scripts de Drizzle para facilitar el desarrollo:' },
    ...
    "studio": "drizzle-kit studio"
        ...
        { type: 'subtitle', text: 'Paso 2: Definir el Esquema y Configurar Drizzle' },
        { type: 'paragraph', text: 'Ahora, crearemos la estructura de nuestra base de datos. Crea una carpeta `db` y dentro, un archivo `schema.ts`.' },
...
`},
        { type: 'paragraph', text: 'Luego, crea el archivo de configuración `drizzle.config.ts` en la raíz del proyecto:' },
        { type: 'code', language: 'typescript', code: `
// drizzle.config.ts
...
export default defineConfig({
  ...
  out: './drizzle',
        ...
        { type: 'paragraph', text: 'Con el esquema y la configuración listos, genera tu primera migración:' },
        { type: 'code', language: 'bash', code: 'npm run generate' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 3: Conexión y Migración en la App' },
        { type: 'paragraph', text: 'Crea un archivo `db/index.ts` para centralizar la conexión a la base de datos.'},
        { type: 'code', language: 'typescript', code: `
// db/index.ts
...
export const db = drizzle(expoDb, { schema });`
        ...
        { type: 'paragraph', text: 'Ahora, configura tu layout raíz (`app/_layout.tsx`) para aplicar las migraciones al iniciar la aplicación. Esto asegura que la base de datos siempre esté actualizada.'},
  ...
  return (
    <Stack>
      <Stack.Screen name="index" options={{ title: 'Mi Lista de Tareas' }} />
        ...
        { type: 'callout', alertType: 'warning', text: 'Recuerda configurar `metro.config.js` y `babel.config.js` para importar archivos `.sql` como se explicó en la Conferencia 21.'},
        { type: 'divider' },

        { type: 'subtitle', text: 'Paso 4: Construcción de la UI y Lógica CRUD' },
        { type: 'paragraph', text: 'Reemplaza el contenido de `app/index.tsx` con la lógica completa de la aplicación. Aquí implementaremos las funciones para leer, crear, actualizar y borrar tareas, conectándolas a la interfaz de usuario.'},
