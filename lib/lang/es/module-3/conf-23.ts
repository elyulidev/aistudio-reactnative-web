...
export const conference23: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Objetivo: Completar el ciclo CRUD aprendiendo a modificar registros existentes con la operación de actualización y a eliminarlos permanentemente de la base de datos.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Actualizando Datos (UPDATE)' },
      { type: 'paragraph', text: 'Para modificar una o más filas que ya existen en una tabla, usamos el método `db.update()`.' },
...
`},
      ...
      { type: 'paragraph', text: 'Para eliminar filas de una tabla, usamos el método `db.delete()`. Esta operación es permanente y debe usarse con cuidado.' },
      { type: 'code', language: 'typescript', code: `
import { db } from './db';
import { tasks } from './db/schema';
import { eq } from 'drizzle-orm';
...
`},
      ...
      { type: 'paragraph', text: '¿Qué pasa si necesitas realizar varias operaciones de base de datos y quieres que todas tengan éxito o que ninguna lo tenga? Por ejemplo, transferir dinero implica restar de una cuenta y sumar a otra. Si una de las dos operaciones falla, la base de datos quedaría en un estado inconsistente.' },
      { type: 'paragraph', text: 'Una **transacción** agrupa múltiples operaciones como una sola unidad atómica. Drizzle lo maneja con `db.transaction(async (tx) => { ... })`.' },
      { type: 'code', language: 'typescript', code: `
...
`},
      { type: 'callout', alertType: 'info', text: 'Las transacciones son un concepto más avanzado, pero es bueno saber que existen para garantizar la consistencia de los datos en operaciones complejas.' },
      { type: 'divider' },
      ...
      { 
        type: 'quiz',
        questions: [
            { "question": "¿Qué método se utiliza para modificar un registro existente en Drizzle?", "options": ["db.modify()", "db.change()", "db.update()", "db.set()"], "correctAnswer": 2 },
            { "question": "En una operación `db.update(tasks).set({ ... })`, ¿qué sucede si omites la cláusula `.where()`?", "options": ["Drizzle lanza un error.", "No se actualiza ninguna fila.", "Se actualiza solo la primera fila.", "Se actualizan todas las filas de la tabla."], "correctAnswer": 3 },
            { "question": "El método para eliminar una fila de una tabla es:", "options": ["db.remove()", "db.destroy()", "db.erase()", "db.delete()"], "correctAnswer": 3 },
            { "question": "¿Para qué se utiliza una transacción en el contexto de una base de datos?", "options": ["Para agrupar múltiples operaciones como una sola unidad atómica, asegurando que todas tengan éxito o ninguna lo tenga.", "Para acelerar las consultas de lectura.", "Para hacer una copia de seguridad de la base de datos.", "Para encriptar los datos antes de guardarlos."], "correctAnswer": 0 }
        ]
      }
    ]
};