...
export const conference18: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Objetivo: Instalar y configurar Drizzle ORM y Drizzle Kit para un proyecto Expo con una base de datos SQLite, sentando las bases para la gestión de datos locales.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Instalación de las Librerías Necesarias' },
      { type: 'paragraph', text: 'Para trabajar con Drizzle ORM y Expo SQLite, necesitaremos instalar dos paquetes principales de Drizzle, además de `expo-sqlite` (que debimos haber instalado en la conferencia anterior).' },
      { type: 'list', items: [
      ...
      { type: 'code', language: 'bash', code: `npm install drizzle-orm\nnpm install -D drizzle-kit` },
      { type: 'paragraph', text: 'También debemos asegurarnos de que `expo-sqlite` esté instalado:' },
      { type: 'code', language: 'bash', code: 'npx expo install expo-sqlite' },
      { type: 'callout', alertType: 'tip', text: 'Después de la instalación, puedes verificar las versiones en tu archivo `package.json`.' },
      { type: 'divider' },

      { type: 'subtitle', text: '2. Configuración de `drizzle.config.ts`' },
      { type: 'paragraph', text: 'El archivo `drizzle.config.ts` es crucial, ya que contiene toda la información que Drizzle Kit necesita para entender cómo interactuar con tu base de datos. Debes crearlo en la raíz de tu proyecto.' },
...
`},
    { type: 'list', items: [
      ...
      { type: 'paragraph', text: 'Asegúrate de crear las carpetas y el archivo que se mencionan, por ejemplo, `db/schema.ts`.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: '3. Creación de Scripts NPM' },
      { type: 'paragraph', text: 'Para simplificar la ejecución de los comandos de Drizzle Kit, los añadimos como scripts en nuestro `package.json`.' },
...
`},
      ...
      { type: 'paragraph', text: 'Ahora puedes ejecutar `npm run generate` o `npm run studio` desde tu terminal.' },
      { type: 'divider' },

      { type: 'subtitle', text: '4. Consideraciones de Desarrollo vs. Producción' },
      { type: 'paragraph', text: 'Es importante entender cómo manejamos la base de datos en los diferentes entornos:' },
      ...
      { 
        type: 'quiz',
        questions: [
            { "question": "¿Qué paquete de Drizzle es una dependencia de desarrollo y se usa para generar migraciones?", "options": ["drizzle-orm", "drizzle-kit", "expo-drizzle", "sqlite-drizzle"], "correctAnswer": 1 },
            { "question": "En `drizzle.config.ts`, ¿qué propiedad especifica la carpeta donde se guardarán los archivos de migración?", "options": ["schema", "driver", "dialect", "out"], "correctAnswer": 3 },
            { "question": "¿Qué script de npm usarías para lanzar la interfaz visual de Drizzle para tu base de datos?", "options": ["npm run generate", "npm run migrate", "npm run studio", "npm run start"], "correctAnswer": 2 },
            { "question": "Para usar Drizzle con Expo, ¿qué valor debe tener la propiedad `driver` en la configuración?", "options": ["sqlite", "expo-sqlite", "expo", "react-native"], "correctAnswer": 2 }
        ]
      }
    ]
};