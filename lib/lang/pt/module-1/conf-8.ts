...
export const conference8: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'É hora de colocar em prática tudo o que aprendemos no Módulo 1! Neste mini projeto, construiremos um componente de UI reutilizável e visualmente atraente: um cartão de filme. Este exercício consolidará seus conhecimentos sobre componentes, layout com Flexbox, estilização com NativeWind e gerenciamento de ativos.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/movie-card-project-goal.png', caption: 'O componente `MovieCard` que iremos construir.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Passo 1: Preparação do Componente e Ativos' },
      { type: 'paragraph', text: 'Primeiro, vamos criar o arquivo para o nosso novo componente. Dentro do seu projeto, crie uma pasta `components` (se ainda não existir) e, dentro dela, um novo arquivo chamado `MovieCard.tsx`.' },
      { type: 'paragraph', text: 'Copie o seguinte código base para o seu novo arquivo. Este será o nosso ponto de partida.' },
...
` },
      { type: 'callout', alertType: 'info', text: 'Para a imagem do cartaz, usaremos uma URL. Num projeto real, você poderia obter estes dados de uma API ou dos seus ativos locais.' },
      
      { type: 'divider' },

      { type: 'subtitle', text: 'Passo 2: Estrutura Principal e Estilo do Contentor' },
      { type: 'paragraph', text: 'Vamos começar pelo contentor principal. Queremos que ele tenha um fundo escuro, cantos arredondados e uma sombra para que pareça um cartão. Usaremos o NativeWind para isso.' },
      { type: 'code', language: 'jsx', code: `
// ... (importações e dados)
...
`},
      ...
      { type: 'paragraph', text: 'Agora, vamos adicionar a imagem do cartaz dentro do nosso contentor. Faremos com que ocupe toda a largura do cartão e daremos uma altura fixa.' },
      { type: 'code', language: 'jsx', code: `
// ...
...
`},
      ...
      { type: 'paragraph', text: 'Abaixo da imagem, criaremos uma secção com o título, o gênero e a classificação. Usaremos uma `<View>` com preenchimento e flexbox para organizar o conteúdo.' },
      { type: 'code', language: 'jsx', code: `
// ...
...
` },
      ...
      { type: 'paragraph', text: 'Para completar o nosso cartão, adicionaremos um botão "Ver Detalhes". Usaremos o componente `<Pressable>` para que seja interativo e estilizá-lo-emos para que se destaque.' },
      { type: 'code', language: 'jsx', code: `
...
`},
      ...
      { type: 'paragraph', text: 'Parabéns! Você construiu um componente complexo e reutilizável. Aqui está o código completo para `MovieCard.tsx`:' },
      { type: 'code', language: 'jsx', code: `
import React from 'react';
import { View, Text, Image, Pressable } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
...
export default MovieCard;
`},
      { type: 'paragraph', text: 'Para vê-lo em ação, importe e use o seu novo componente na sua tela principal (por exemplo, `app/index.tsx`):' },
      { type: 'code', language: 'jsx', code: `
...
`},
      { type: 'paragraph', text: 'Este mini projeto demonstra como combinar diferentes componentes e utilitários de estilo para criar interfaces de utilizador polidas e profissionais. Excelente trabalho!' }
    ]
};
