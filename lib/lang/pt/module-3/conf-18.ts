...
export const conference18: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Objetivo: Instalar e configurar o Drizzle ORM e o Drizzle Kit para um projeto Expo com um banco de dados SQLite, estabelecendo as bases para o gerenciamento de dados locais.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Instalação das Bibliotecas Necessárias' },
      { type: 'paragraph', text: 'Para trabalhar com o Drizzle ORM e o Expo SQLite, precisaremos instalar dois pacotes principais do Drizzle, além do `expo-sqlite` (que deveríamos ter instalado na conferência anterior).' },
      { type: 'list', items: [
      ...
      { type: 'paragraph', text: 'Também devemos garantir que o `expo-sqlite` esteja instalado:' },
      { type: 'code', language: 'bash', code: 'npx expo install expo-sqlite' },
      { type: 'callout', alertType: 'tip', text: 'Após a instalação, você pode verificar as versões no seu arquivo `package.json`.' },
      { type: 'divider' },

      { type: 'subtitle', text: '2. Configuração do `drizzle.config.ts`' },
      { type: 'paragraph', text: 'O arquivo `drizzle.config.ts` é crucial, pois contém todas as informações que o Drizzle Kit precisa para entender como interagir com seu banco de dados. Você deve criá-lo na raiz do seu projeto.' },
...
`},
    { type: 'list', items: [
      ...
      { type: 'paragraph', text: 'Certifique-se de criar as pastas e o arquivo mencionados, por exemplo, `db/schema.ts`.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: '3. Criação de Scripts NPM' },
      { type: 'paragraph', text: 'Para simplificar a execução dos comandos do Drizzle Kit, nós os adicionamos como scripts no nosso `package.json`.' },
...
`},
      ...
      { type: 'paragraph', text: 'Agora você pode executar `npm run generate` ou `npm run studio` no seu terminal.' },
      { type: 'divider' },

      { type: 'subtitle', text: '4. Considerações de Desenvolvimento vs. Produção' },
      { type: 'paragraph', text: 'É importante entender como gerenciamos o banco de dados nos diferentes ambientes:' },
      ...
      { 
        type: 'quiz',
        questions: [
            { "question": "Qual pacote do Drizzle é uma dependência de desenvolvimento e é usado para gerar migrações?", "options": ["drizzle-orm", "drizzle-kit", "expo-drizzle", "sqlite-drizzle"], "correctAnswer": 1 },
            { "question": "No `drizzle.config.ts`, qual propriedade especifica a pasta onde os arquivos de migração serão salvos?", "options": ["schema", "driver", "dialect", "out"], "correctAnswer": 3 },
            { "question": "Qual script npm você usaria para iniciar la interface visual do Drizzle para o seu banco de dados?", "options": ["npm run generate", "npm run migrate", "npm run studio", "npm run start"], "correctAnswer": 2 },
            { "question": "Para usar o Drizzle com o Expo, qual valor a propriedade `driver` deve ter na configuração?", "options": ["sqlite", "expo-sqlite", "expo", "react-native"], "correctAnswer": 2 }
        ]
      }
    ]
};