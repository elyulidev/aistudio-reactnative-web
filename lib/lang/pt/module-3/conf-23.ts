...
export const conference23: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Objetivo: Completar o ciclo CRUD aprendendo a modificar registos existentes com a operação de atualização e a eliminá-los permanentemente da base de dados.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Atualizando Dados (UPDATE)' },
      { type: 'paragraph', text: 'Para modificar uma ou mais linhas que já existem numa tabela, usamos o método `db.update()`.' },
...
`},
      ...
      { type: 'paragraph', text: 'Para eliminar linhas de uma tabela, usamos o método `db.delete()`. Esta operação é permanente e deve ser usada com cuidado.' },
      { type: 'code', language: 'typescript', code: `
import { db } from './db';
import { tasks } from './db/schema';
import { eq } from 'drizzle-orm';
...
`},
      { type: 'callout', alertType: 'info', text: 'As transações são um conceito mais avançado, mas é bom saber que existem para garantir a consistência dos dados em operações complexas.' },
      { type: 'divider' },
      ...
      { 
        type: 'quiz',
        questions: [
            { "question": "Qual método é utilizado para modificar um registo existente no Drizzle?", "options": ["db.modify()", "db.change()", "db.update()", "db.set()"], "correctAnswer": 2 },
            { "question": "Numa operação `db.update(tasks).set({ ... })`, o que acontece se omitir a cláusula `.where()`?", "options": ["O Drizzle lança um erro.", "Nenhuma linha é atualizada.", "Apenas a primeira linha é atualizada.", "Todas as linhas da tabela são atualizadas."], "correctAnswer": 3 },
            { "question": "O método para eliminar uma linha de uma tabela é:", "options": ["db.remove()", "db.destroy()", "db.erase()", "db.delete()"], "correctAnswer": 3 },
            { "question": "Para que é utilizada uma transação no contexto de uma base de dados?", "options": ["Para agrupar múltiplas operações como uma única unidade atômica, garantindo que todas sejam bem-sucedidas ou nenhuma o seja.", "Para acelerar as consultas de leitura.", "Para fazer uma cópia de segurança da base de dados.", "Para encriptar os dados antes de os guardar."], "correctAnswer": 0 }
        ]
      }
    ]
};