...
export const conference40: CurriculumTopic = {
    ...
    content: [
        ...
        { type: 'paragraph', text: 'Escrever código que funciona é apenas o primeiro passo. Escrever código limpo — código que é legível, de fácil manutenção e modificação — é uma habilidade essencial para o desenvolvimento profissional. A refatoração é o processo de reestruturar o código existente, sem alterar o seu comportamento externo, para melhorar estes atributos.' },
        { type: 'divider' },
        ...
        { type: 'paragraph', text: 'Os "Code Smells" (maus cheiros no código) são sintomas de que algo pode estar errado no seu código e poderia beneficiar de uma refatoração.' },
        { type: 'list', items: [
        ...
        { type: 'subtitle', text: '3. Exercício Prático: Refatorando um Ecrã de Pesquisa' },
        { type: 'paragraph', text: 'Vamos pegar no ecrã de pesquisa de filmes do módulo anterior. Embora funcional, mistura a lógica da API, a gestão de estado e a renderização da UI num único componente. Vamos refatorá-lo.' },
        
        { type: 'paragraph', text: '**ANTES: O Componente Monstruoso**' },
        { type: 'paragraph', text: 'Tudo está em `app/index.tsx`. É difícil de ler e ainda mais difícil de reutilizar qualquer parte da sua lógica.' },
...
`},
        ...
        { type: 'paragraph', text: 'Separaremos as responsabilidades em ficheiros mais pequenos e manejáveis.' },

        { type: 'paragraph', text: '**1. Mover a Lógica para um Hook Personalizado (`hooks/useMovieSearch.ts`)**' },
...
`},
        { type: 'paragraph', text: '**2. Criar um Componente de UI Reutilizável (`components/MovieListItem.tsx`)**' },
        { type: 'paragraph', text: 'Isto já fizemos no projeto do módulo 4, demonstrando que já estávamos a aplicar bons princípios.'},

        { type: 'paragraph', text: '**3. O Componente do Ecrã Agora é Limpo e Declarativo**' },
        { type: 'code', language: 'tsx', code: `
...
`},
        { type: 'callout', alertType: 'tip', text: 'Observe como o ecrã agora só se preocupa com a UI e em ligar o estado do hook aos componentes. A lógica de pesquisa é completamente independente e poderia ser reutilizada noutra parte da app.'}
    ]
};
