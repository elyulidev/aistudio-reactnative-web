...
`},
        { type: 'paragraph', text: 'Siga os passos das conferências anteriores para configurar o Drizzle (`drizzle.config.ts`, `package.json scripts`), o NativeWind (`tailwind.config.js`, `babel.config.js`, `global.css`), e a importação de CSS no `app/_layout.tsx`.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Passo 2: Definir o Esquema da Base de Dados' },
        { type: 'paragraph', text: 'Crie o ficheiro `db/schema.ts` para definir a tabela de propriedades.' },
...
`},
        ...
        { type: 'paragraph', text: 'Crie um ficheiro `db/seed.ts` para popular a base de dados com algumas propriedades de exemplo. Isto é crucial para poder desenvolver a UI sem ter de adicionar dados manualmente.' },
        { type: 'code', language: 'typescript', code: `
// db/seed.ts
import { db } from './';
...
`},
        { type: 'callout', alertType: 'info', text: 'Para as imagens, usaremos URLs do Unsplash por simplicidade. Na segunda parte, iremos substituí-las por imagens do sistema de ficheiros local.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Passo 4: Construir o Ecrã de Listagem (`app/index.tsx`)' },
        { type: 'paragraph', text: 'Este ecrã mostrará os filtros e a lista de propriedades.' },
...
`},
        { type: 'paragraph', text: 'Na próxima conferência, construiremos o componente `PropertyCard`, o ecrã de detalhes e o formulário para adicionar novas propriedades.' }
    ]
};
