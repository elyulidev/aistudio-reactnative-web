...
export const conference46: CurriculumTopic = {
    ...
    content: [
        ...
        { type: 'paragraph', text: 'Neste projeto, construiremos um protótipo de uma aplicação de mídia social. Criaremos um feed onde poderão ser exibidas publicações com texto e imagem, e os utilizadores poderão dar "like". Toda a informação, incluindo as publicações e os likes, será armazenada e gerida localmente na base de dados SQLite com Drizzle.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Passo 1: Definir o Esquema da Base de Dados' },
        { type: 'paragraph', text: 'Precisamos de duas tabelas: uma para as publicações e outra para registar os likes de cada publicação. A relação será "uma publicação pode ter muitos likes".' },
...
`},
        { type: 'paragraph', text: 'Não se esqueça de gerar a nova migração: `npm run generate`.' },
        { type: 'divider' },

        { type: 'subtitle', text: 'Passo 2: Criar o Formulário de Nova Publicação' },
        { type: 'paragraph', text: 'Crie um novo ecrã modal para que os utilizadores possam criar publicações. Use `expo-image-picker` e `expo-file-system` para lidar com a seleção e o armazenamento de imagens.' },
...
`},
        ...
        { type: 'paragraph', text: 'O ecrã principal (`app/index.tsx`) buscará todas as publicações e os seus likes, e exibi-los-á numa `FlatList`.' },
        
        { type: 'paragraph', text: '**A Consulta com `with` para Relações:**' },
        { type: 'paragraph', text: 'Para obter as publicações e os seus likes relacionados de forma eficiente, usamos a sintaxe `with` do Drizzle, que realiza um JOIN por baixo.' },
        { type: 'code', language: 'typescript', code: `
// No seu ecrã de feed
...
`},
        ...
        { type: 'paragraph', text: 'Crie um componente reutilizável para exibir cada publicação. Este componente também tratará da lógica de dar "like".' },
        { type: 'code', language: 'tsx', code: `
...
`},
        { type: 'callout', alertType: 'tip', text: 'Passar uma função `refetch` como prop ao `PostCard` é uma forma simples de fazer com que o feed se atualize imediatamente após dar like. Em aplicações mais complexas, seriam usadas soluções de gestão de estado mais sofisticadas.'}
    ]
};
