...
export const conference33: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Nesta segunda parte do projeto, construiremos a tela de detalhes para um filme selecionado e adicionaremos uma funcionalidade chave: guardar filmes como "favoritos" na nossa base de dados local SQLite usando o Drizzle ORM.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Passo 1: Esquema e Migração para Favoritos' },
      { type: 'paragraph', text: 'Primeiro, precisamos de uma tabela na nossa base de dados para armazenar os filmes favoritos. Vamos abrir `db/schema.ts` e adicionar uma nova tabela.' },
...
`},
      { type: 'paragraph', text: 'Usamos o ID do filme da API como chave primária para evitar duplicados. Agora, gere a migração:' },
      { type: 'code', language: 'bash', code: 'npm run generate' },
      { type: 'callout', alertType: 'info', text: 'Lembre-se que as migrações serão aplicadas automaticamente ao reiniciar a app graças ao hook `useMigrations` no seu layout raiz.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: 'Passo 2: Criar a Rota Dinâmica para Detalhes' },
      { type: 'paragraph', text: 'Precisamos de uma nova tela que possa mostrar os detalhes de qualquer filme. Criamos uma rota dinâmica em `app/movie/[id].tsx`.' },
...
...
`},
      { type: 'paragraph', text: 'Registe esta nova rota no seu Stack Navigator raiz (`app/_layout.tsx`).' },
...
`},
      ...
      { type: 'paragraph', text: 'Vamos modificar o nosso `MovieListItem` para que, ao ser pressionado, navegue para a tela de detalhes, passando o ID do filme.' },
      { type: 'code', language: 'tsx', code: `
...
`},
      ...
      { type: 'subtitle', text: 'Passo 4: Construir a Tela de Detalhes e a Lógica de Favoritos' },
      { type: 'paragraph', text: 'Esta é a parte central. Em `app/movie/[id].tsx`, obteremos o ID, carregaremos os dados do filme (poderíamos fazer outra chamada à API para mais detalhes, mas por simplicidade, passaremos os dados básicos), e trataremos da lógica de favoritos.' },
      { type: 'code', language: 'tsx', code: `
// app/movie/[id].tsx
...
`},
      { type: 'callout', alertType: 'tip', text: 'O código está comentado nas partes que exigiriam uma chamada à API completa para obter os detalhes, mas a lógica da base de dados para adicionar/remover favoritos está completa. Para uma implementação total, precisaria de passar os dados do filme através do router ou fazer uma nova chamada à API.'},
    ]
};
