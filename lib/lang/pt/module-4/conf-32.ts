...
export const conference32: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Neste projeto prático, construiremos a primeira parte de uma aplicação para procurar filmes. Integraremos uma API externa (TheMovieDB), implementaremos uma tela de busca com `TextInput` e `debouncing`, e exibiremos os resultados de forma eficiente usando `FlatList`. Este projeto consolidará tudo o que foi aprendido sobre consumo de APIs e gestão de listas.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/movie-app-search.png', caption: 'A tela de busca de filmes que construiremos.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Parte 1: Obter uma Chave de API e Configurar o Ambiente' },
      { type: 'paragraph', text: 'Para nos comunicarmos com o The Movie Database (TMDB), primeiro, você precisa de uma chave de API. É grátis e essencial para o nosso projeto.' },
      { type: 'list', items: [
...
`},
      { type: 'callout', alertType: 'warning', text: 'Certifique-se de adicionar o ficheiro `.env` ao seu `.gitignore` para não enviar as suas chaves para repositórios públicos. Depois de criar ou modificar o ficheiro `.env`, deve reiniciar o servidor de desenvolvimento do Expo com o comando `npx expo start --clear` para que as alterações entrem em vigor.' },
      { type: 'divider' },
      
      { type: 'subtitle', text: 'Parte 2: Criar o Serviço da API' },
      { type: 'paragraph', text: 'É uma boa prática encapsular toda a lógica das chamadas à API num único local. Crie uma nova pasta `services` e, dentro dela, um ficheiro `api.ts`.' },
...
`},
    ...
    { type: 'paragraph', text: 'Para evitar fazer uma chamada à API cada vez que o utilizador prime uma tecla, usaremos a técnica de "debouncing". Criaremos um hook personalizado para isto. Crie uma pasta `hooks` e dentro dela um ficheiro `useDebounce.ts`.'},
    { type: 'code', language: 'typescript', code: `
...
`},
      ...
      { type: 'paragraph', text: 'Para manter o nosso código limpo, criaremos um componente separado para cada item da lista. Crie uma pasta `components` e dentro dela um ficheiro `MovieListItem.tsx`.' },
...
`},
      ...
      { type: 'paragraph', text: 'Finalmente, montamos tudo na nossa tela principal. Modifique o seu `app/index.tsx` para conter toda a lógica de estado e a UI da busca.' },
      { type: 'code', language: 'tsx', code: `
// app/index.tsx
...
`},
    { type: 'paragraph', text: 'E está pronto! Agora tem uma tela de busca funcional. Ao escrever, a aplicação esperará 500ms, chamará a API do TMDB e exibirá os resultados numa lista virtualizada, gerindo os estados de carregamento e erro. Na próxima conferência, adicionaremos a navegação para uma tela de detalhes.' }
    ]
};
