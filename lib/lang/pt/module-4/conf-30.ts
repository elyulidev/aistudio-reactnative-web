...
export const conference30: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: '`FlatList` utiliza uma estratégia chamada **virtualização**. Em vez de renderizar toda a lista, `FlatList` apenas renderiza os itens que estão visíveis no ecrã num determinado momento, mais alguns itens adicionais fora do ecrã para que a rolagem seja fluida. À medida que o utilizador rola, os itens que saem do ecrã são "desmontados" (reciclando as suas vistas nativas) e os novos itens que entram são renderizados.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/flatlist-virtualization.png', caption: 'FlatList apenas mantém em memória os itens visíveis e um pequeno buffer.' },
      { type: 'divider' },

      { type: 'subtitle', text: '3. Propriedades Essenciais do `FlatList`' },
      { type: 'paragraph', text: 'Para usar `FlatList`, precisa de fornecer pelo menos três propriedades chave:' },
...
`},
      ...
      { type: 'subtitle', text: '4. Gerindo Estados Vazios com `ListEmptyComponent`' },
      { type: 'paragraph', text: 'O que acontece se o seu array `data` estiver vazio? Por defeito, `FlatList` não mostrará nada. Para uma melhor experiência do utilizador, podemos fornecer um componente para ser renderizado neste caso, usando a prop `ListEmptyComponent`.' },
...
`},
      ...
      { type: 'heading', text: 'Teste os seus conhecimentos!' },
        { 
          ...
          questions: [
            {
              "question": "Qual é o principal problema de desempenho ao usar `ScrollView` com um `.map()` para uma lista muito longa?",
              "options": ["Renderiza todos os itens da lista de uma só vez, consumindo muita memória.", "Não permite rolagem horizontal.", "É difícil de estilizar.", "Não consegue lidar com dados assíncronos."],
              "correctAnswer": 0
            },
            {
              "question": "A técnica que `FlatList` utiliza para renderizar apenas os itens visíveis no ecrã chama-se:",
              "options": ["Lazy Loading", "Virtualização", "Memoização", "Renderização Condicional"],
              "correctAnswer": 1
            },
            {
              "question": "Qual prop do `FlatList` é responsável por definir como cada item da lista deve ser renderizado?",
              "options": ["`data`", "`component`", "`keyExtractor`", "`renderItem`"],
              "correctAnswer": 3
            },
            {
              "question": "Se o seu array de dados estiver vazio, que prop do `FlatList` pode usar para mostrar uma mensagem ao utilizador?",
