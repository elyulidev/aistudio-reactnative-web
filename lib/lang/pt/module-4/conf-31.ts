...
export const conference31: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Além da renderização básica, o `FlatList` oferece um conjunto potente de funcionalidades para criar experiências de utilizador modernas e fluidas, como o carregamento infinito de dados, a capacidade de atualizar a lista e a adição de cabeçalhos e rodapés.' },
      { type: 'divider' },

      { type: 'subtitle', text: '1. Carregamento Infinito (Infinite Scrolling) com `onEndReached`' },
      { type: 'paragraph', text: 'Em vez de carregar milhares de itens de uma só vez, uma estratégia melhor é carregar um lote inicial (ex. 20 itens) e carregar mais à medida que o utilizador se desloca para o final da lista. Isto é alcançado com a prop `onEndReached`.' },
      { type: 'list', items: [
...
`},
      ...
      { type: 'subtitle', text: '2. "Puxar para Atualizar" (Pull to Refresh) com `RefreshControl`' },
      { type: 'paragraph', text: 'Uma experiência de utilizador comum em aplicações móveis é a capacidade de "puxar" do topo de uma lista para recarregar os dados. `FlatList` suporta isto através das props `refreshing` e `onRefresh`.' },
      { type: 'code', language: 'tsx', code: `
import React, { useState, useCallback } from 'react';
...
`},
      ...
      { type: 'subtitle', text: '3. Cabeçalhos e Rodapés (`ListHeaderComponent`, `ListFooterComponent`)' },
      { type: 'paragraph', text: '`FlatList` permite adicionar componentes React no topo e na base da lista. Estes componentes deslocam-se juntamente com a lista.' },
      { type: 'list', items: [
...
`},
      ...
      { type: 'heading', text: 'Teste os seus conhecimentos!' },
        { 
          ...
          questions: [
            {
              "question": "Qual prop do `FlatList` é utilizada para implementar o carregamento infinito (infinite scroll)?",
              "options": ["`onScrollEnd`", "`onLoadMore`", "`onEndReached`", "`onScroll`"],
              "correctAnswer": 2
            },
            {
              "question": "Para implementar a funcionalidade de 'puxar para atualizar', que props do `FlatList` são essenciais?",
              "options": ["`loading` e `onLoad`", "`refreshing` e `onRefresh`", "`pulling` e `onPull`", "`reloading` e `onReload`"],
              "correctAnswer": 1
            },
            {
              "question": "Se quiser adicionar uma barra de pesquisa que se desloque juntamente com o conteúdo da lista, que prop do `FlatList` deve usar?",
              "options": ["`Header`", "`TopComponent`", "`ListHeaderComponent`", "Simplesmente colocá-lo antes do `FlatList` no JSX"],
              "correctAnswer": 2
            },
            {
              "question": "A prop `onEndReachedThreshold` recebe um valor numérico. O que representa um valor de 0.5?",
              "options": ["A função `onEndReached` será chamada quando o utilizador tiver percorrido 50% da lista.", "A função `onEndReached` será chamada quando o final do conteúdo estiver a uma distância de meia altura da parte visível da lista.", "A função `onEndReached` esperará 0.5 segundos antes de ser ativada.", "A função `onEndReached` será chamada quando restarem 50 itens para mostrar."],
              "correctAnswer": 1
            }
          ]
        }
