...
export const conference14: CurriculumTopic = {
    ...
    content: [
      ...
      { type: 'paragraph', text: 'Um caso de uso muito comum é ter um estado booleano que alterna (como para um modal ou um menu pendente). Em vez de escrever `const [isOpen, setIsOpen] = useState(false);` e a função `toggle` cada vez, podemos criar um hook para isso.' },
      { type: 'paragraph', text: 'Crie uma pasta `hooks` na raiz do seu projeto e, dentro dela, um ficheiro `useToggle.ts`.' },
...
`},
    { type: 'paragraph', text: 'Agora, em qualquer componente, podemos usá-lo de forma muito mais limpa:' },
...
`},
      ...
      { type: 'paragraph', text: 'Numa barra de pesquisa, não queremos fazer um pedido à API a cada letra que o utilizador escreve. Queremos esperar que o utilizador termine de escrever. O "debouncing" é a técnica perfeita para isto, e podemos encapsulá-la num hook.' },
      { type: 'code', language: 'typescript', code: `
...
`},
    { type: 'paragraph', text: 'É assim que o usaríamos num componente de pesquisa:' },
    { type: 'code', language: 'jsx', code: `
...
`},
    ...
    { type: 'subtitle', id: 'async-storage', text: '4. Uma Olhada no Armazenamento Local: AsyncStorage' },
    { type: 'paragraph', text: 'Antes da nossa tarefa prática, precisamos de uma forma de guardar dados no dispositivo do utilizador. Para dados simples, como as preferências do utilizador ou um token de sessão, o `AsyncStorage` é a ferramenta perfeita. É um sistema de armazenamento de chave-valor, assíncrono e persistente.' },
    { type: 'list', items: [
...
`},
    { type: 'callout', alertType: 'tip', text: 'Com este conhecimento sobre `AsyncStorage`, agora tem tudo o que precisa para abordar a seguinte tarefa prática.' },

    { type: 'subtitle', text: '5. Tarefa Prática: Criar um Hook `usePersistentState`' },
    { type: 'paragraph', text: 'Vamos criar um hook que atue como o `useState`, mas que também guarde o valor no armazenamento local do dispositivo usando o `AsyncStorage`. Isto é incrivelmente útil para lembrar as preferências do utilizador, como o tema da aplicação.' },
    { 
        type: 'assignment',
