...
export const conference16: CurriculumTopic = {
    ...
    content: [
      { type: 'heading', text: 'Mini Projeto: App de Blog com Navegação Completa' },
      { type: 'paragraph', text: 'É hora de consolidar tudo o que aprendemos no Módulo 2! Neste projeto, construiremos uma aplicação de blog simples, mas completa. Integraremos um Stack Navigator, um Tab Navigator, rotas dinâmicas para os artigos e uma estrutura de ficheiros lógica usando grupos de rotas.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/blog-app-nav-structure.png', caption: 'Estrutura de navegação da nossa aplicação de blog.' },
      { type: 'divider' },

      { type: 'subtitle', text: 'Passo 1: Estrutura de Ficheiros do Projeto' },
      { type: 'paragraph', text: 'Uma boa estrutura de ficheiros é fundamental. Vamos organizar as nossas rotas da seguinte forma dentro da pasta `app`:' },
...
└── _layout.tsx          # Define o Stack Navigator raiz
      ...
      { type: 'paragraph', text: 'Vamos modificar o `app/_layout.tsx` para definir o nosso Stack principal. Ele irá gerir o grupo de abas e a tela de detalhes dos posts.' },
...
export default function RootLayout() {
  return (
    ...
    { type: 'callout', alertType: 'info', text: 'Ocultamos o cabeçalho para `(tabs)` para que o Tab Navigator possa gerir os seus próprios cabeçalhos, evitando uma barra de título dupla.' },
    { type: 'divider' },
    
    { type: 'subtitle', text: 'Passo 3: Configurar o Tab Navigator' },
    { type: 'paragraph', text: 'Agora, vamos configurar as abas em `app/(tabs)/_layout.tsx`.' },
    { type: 'code', language: 'jsx', code: `
...
export default function TabLayout() {
  return (
    ...
    { type: 'paragraph', text: 'A tela `index` nas nossas abas irá exibir a lista de artigos. Usaremos `FlatList` e `<Link>` para navegar para os detalhes. Para manter as coisas simples, os dados estarão diretamente no ficheiro.' },
    { type: 'code', language: 'jsx', code: `
// app/(tabs)/index.tsx
...
export default function ArticlesScreen() {
  return (
    ...
    { type: 'callout', alertType: 'tip', text: 'O prop `asChild` no componente `<Link>` permite-lhe passar as suas propriedades de navegação para o primeiro componente filho (`Pressable`), tornando toda a área clicável.'},
    { type: 'divider' },

    { type: 'subtitle', text: 'Passo 5: Criar a Tela "Sobre"' },
    { type: 'paragraph', text: 'Esta será uma tela estática simples em `app/(tabs)/about.tsx`.' },
    { type: 'code', language: 'jsx', code: `
// app/(tabs)/about.tsx
...
export default function AboutScreen() {
  return (
    ...
    { type: 'paragraph', text: 'Finalmente, a tela `app/posts/[id].tsx`. Usaremos `useLocalSearchParams` para obter o ID do URL e exibir o artigo correto.' },
    { type: 'code', language: 'jsx', code: `
// app/posts/[id].tsx
import { View, Text } from 'react-native';
...
export default function PostDetailsScreen() {
  ...
  return (
    ...
    { type: 'paragraph', text: 'Você construiu com sucesso uma aplicação com uma estrutura de navegação completa. Combinou um Stack Navigator com um Tab Navigator, usou grupos de rotas para organizar o seu código, criou rotas dinâmicas e passou dados entre telas. Este é o fundamento de quase qualquer aplicação móvel complexa!' }
    ]
};
