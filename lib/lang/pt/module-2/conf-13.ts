...
`},
    ...
    { type: 'paragraph', text: '`useEffect` permite-nos executar código que interage com o mundo exterior: pedidos a APIs, subscrições, etc. É executado depois de o componente ser renderizado.' },
    { type: 'code', language: 'jsx', code: `
...
`},
      ...
      { type: 'paragraph', text: 'À medida que uma aplicação cresce, muitas vezes precisamos de partilhar dados entre componentes que estão muito separados na árvore de componentes. Passar props através de muitos níveis, conhecido como **"prop drilling"**, torna-se complicado.' },
      { type: 'image', imageUrl: 'https://storage.googleapis.com/aistudio-hosting/bootcamp-assets/prop-drilling.png', caption: '"Prop drilling": O componente pai passa props através de um componente intermédio para chegar ao neto.' },
      { type: 'paragraph', text: 'A Context API do React resolve este problema criando um "túnel" de dados ao qual qualquer componente descendente pode subscrever, sem necessidade de passar props manualmente.' },
      { type: 'list', items: [
      ...
      { type: 'paragraph', text: 'Vamos construir um sistema de temas (claro/escuro) usando o Context. É um exemplo perfeito de estado global.' },
      
      { type: 'paragraph', text: '**Passo 1: Criar o Contexto**' },
      { type: 'paragraph', text: 'Primeiro, criamos um ficheiro para o nosso contexto, por exemplo, `contexts/ThemeContext.tsx`. Aqui definimos a "forma" do nosso estado e criamos o contexto.' },
...
`},
      ...
      { type: 'paragraph', text: '**Passo 2: Envolver a Aplicação com o Provedor**' },
      { type: 'paragraph', text: 'Para que toda a aplicação tenha acesso ao contexto, envolvemos o nosso layout raiz (`app/_layout.tsx`) com o `ThemeProvider`.' },
      { type: 'code', language: 'jsx', code: `
// app/_layout.tsx
...
export default function RootLayout() {
  return (
    ...
    { type: 'paragraph', text: '**Passo 3: Consumir o Contexto num Componente**' },
    { type: 'paragraph', text: 'Agora, qualquer componente pode aceder ao tema e à função para o alterar usando o nosso hook `useTheme`.' },
    { type: 'code', language: 'jsx', code: `
...
`},
      ...
      { type: 'callout', alertType: 'tip', text: 'A chave é usar a ferramenta certa para o trabalho. `useState` para estado local e `Context API` para estado global que não muda com muita frequência. Se o desempenho se tornar um problema, explore alternativas como o Zustand.' },
      { type: 'divider' },
        ...
        questions: [
          {
            ...
            "options": ["A gestão de efeitos secundários.", "O 'prop drilling' (passar props através de muitos níveis).", "A renderização de listas longas.", "A estilização de componentes."],
            "correctAnswer": 1
          },
          {
            "question": "Em `useEffect(() => { ... }, [])`, o que significa o array de dependências vazio `[]`?",
            "options": ["O efeito é executado em cada renderização.", "O efeito é executado apenas uma vez, quando o componente é montado.", "O efeito nunca é executado.", "O efeito é executado quando o componente é desmontado."],
            "correctAnswer": 1
          },
          {
            "question": "Qual é a principal desvantagem de desempenho da Context API?",
            "options": ["É lento para inicializar.", "Consome muita memória.", "Faz com que todos os componentes consumidores sejam re-renderizados quando o valor do contexto muda.", "Não pode ser usado com `React.memo`."],
            "correctAnswer": 2
          },
          {
            "question": "Para disponibilizar um valor de contexto para toda a sua aplicação, o que deve fazer?",
            "options": ["Exportar o contexto e usá-lo diretamente em cada componente.", "Envolver o componente raiz da sua aplicação (como o layout) com o componente Provedor do contexto.", "Chamar uma função `provideContext()` no seu arquivo principal.", "Configurá-lo no arquivo `app.json`."],
            "correctAnswer": 1
          }
        ]
      }
