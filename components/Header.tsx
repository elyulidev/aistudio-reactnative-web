...
import { ChatIcon, GlobeAltIcon, UserCircleIcon, ArrowRightOnRectangleIcon, ShieldCheckIcon, ChevronDownIcon, Bars3Icon } from './Icons';
...
const UserMenu: React.FC<{ onAdminToggle: () => void; }> = ({ onAdminToggle }) => {
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 py-1 z-20">
                    <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700">
                        <p className="text-sm text-slate-600 dark:text-slate-300">{t('signedInAs')}</p>
...
export const Header: React.FC<HeaderProps> = ({ topicTitle, onChatToggle, isChatOpen, onAdminToggle, onLoginClick, onMenuToggle, children }) => {
    ...
    return (
        <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 sticky top-0 z-10">
            ...
            <div className="flex items-center space-x-2">
                ...
                {session ? <UserMenu onAdminToggle={onAdminToggle} /> : <LoginButton onClick={onLoginClick} />}
            </div>
        </header>
    );
};